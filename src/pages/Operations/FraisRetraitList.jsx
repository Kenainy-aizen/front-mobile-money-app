...
const FraisRetraitList = () => {
  ...
  const validateField = (name, value) => {
    ...
    const newErrors = validateFraisRetraitForm(testData);
  ...
  const fetchAllFrais = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/v1/FraisRecep/all`,
        { headers },
      );
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const validationErrors = validateFraisRetraitForm(formData);
    ...
    try {
      if (isEditMode) {
        await axios.put(
          `http://localhost:8080/api/v1/FraisRecep/FraisRecep/${formData.idRec}/update`,
          formData,
          { headers },
        );
        ...
        await axios.post(
          `http://localhost:8080/api/v1/FraisRecep/add`,
          formData,
          { headers },
        );
        toast.success("Frais de retrait ajoutés !");
      }
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `http://localhost:8080/api/v1/FraisRecep/FraisRecep/${fraisToDelete}/delete`,
        { headers },
      );
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted text-sm italic">
            Configuration des commissions sur les retraits
          </p>
        </div>

        <button
          onClick={openAddModal}
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-card shadow-indigo-900/20 transition-all transform active:scale-95"
        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card w-full max-w-md rounded-3xl border border-card shadow-card-lg animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full bg-input border ${errors.montant1 ? "border-red-500" : "border-card"} rounded-xl p-3 text-white outline-none focus:border-indigo-500`}
                    onChange={(e) => {
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full bg-input border ${errors.montant2 ? "border-red-500" : "border-card"} rounded-xl p-3 text-white outline-none focus:border-indigo-500`}
                    onChange={(e) => {
                      const cleaned = sanitizeNumeric(e.target.value);
              ...
              <div>
                <label className="text-[10px] uppercase font-bold text-muted ml-1 text-indigo-400">
                  Montant des frais (Ar)
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    pattern="[0-9]*"
                    maxLength="9"
                    value={formData.frais_rec}
                    className={`w-full bg-input border ${errors.frais_rec ? "border-red-500" : "border-card"} rounded-xl p-3 pl-10 text-indigo-400 font-bold text-xl outline-none focus:border-indigo-500`}
                    onChange={(e) => {
                      const cleaned = sanitizeNumeric(e.target.value);
                      setFormData({ ...formData, frais_rec: cleaned });
                      validateField("frais_rec", cleaned);
                    }}
                  />
                  {errors.frais_rec && (
                    <p className="text-red-500 text-xs mt-1 ml-1">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-md p-4 ">
          <div className="bg-card w-full max-w-sm rounded-3xl border border-card shadow-card-lg p-8 text-center animate-in zoom-in duration-200">
            ...
            <p className="text-secondary text-sm mb-8">
              Cette modification impactera immédiatement les retraits futurs des
              clients.
            </p>
