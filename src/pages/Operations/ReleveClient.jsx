...
const ReleveClient = () => {
  ...
  const validateField = (name, value) => {
    const testData = { numTel, month, year };
    const updatedData = { ...testData, [name]: value };
    const newErrors = validateReleveForm(updatedData);
    setErrors((prev) => ({ ...prev, [name]: newErrors[name] }));
  };

  const fetchReleve = async () => {
    const validationErrors = validateReleveForm({ numTel, month, year });
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(
      `Date : ${nomMois.charAt(0).toUpperCase() + nomMois.slice(1)} ${year}`,
      105,
      20,
      { align: "center" },
    );
    ...
    // 3. Solde actuel
    doc.text(
      `Solde actuel : ${Number(data.soldeActuel).toLocaleString("de-DE")} Ariary`,
      20,
      70,
    );
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `Total Débit : ${Number(data.totalDebit).toLocaleString("de-DE")} Ar`,
      20,
      finalY,
    );
    doc.text(
      `Total Crédit : ${Number(data.totalCredit).toLocaleString("de-DE")} Ar`,
      20,
      finalY + 8,
    );
  ...
  return (
    <div className="ml-64 p-8 bg-main min-h-screen flex-1 text-primary">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-heading">Relevé de Compte</h1>
      ...
      <div className="bg-card p-6 rounded-2xl border border-card shadow-xl mb-8 flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px]">
          <label className="text-[10px] uppercase font-bold text-secondary ml-1">
            Numéro Client
          </label>
          <div className="relative">
            ...
            <input
              ...
              inputMode="numeric"
              pattern="[0-9]*"
              className={`w-full bg-input border ${errors.numTel ? "border-red-500" : "border-card"} rounded-xl p-3 pl-10 text-heading outline-none focus:border-blue-500 transition-colors`}
              value={numTel}
              onChange={(e) => {
            ...
            />
            {errors.numTel && (
              <p className="text-red-500 text-xs mt-1 ml-1">{errors.numTel}</p>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <select
            className="w-full bg-input border border-card rounded-xl p-3 text-heading outline-none transition-colors"
            value={month}
        ...
        <div>
          ...
          <input
            ...
            pattern="[0-9]*"
            maxLength="4"
            className={`w-full bg-input border ${errors.year ? "border-red-500" : "border-card"} rounded-xl p-3 pl-10 text-heading outline-none focus:border-blue-500 transition-colors`}
            value={year}
            onChange={(e) => {
              const cleaned = sanitizeNumeric(e.target.value).slice(0, 4);
              setYear(cleaned);
              validateField("year", cleaned);
            }}
          />
          {errors.year && (
      ...
      {data && (
        <div className="bg-white text-black p-10 rounded-lg max-w-4xl mx-auto shadow-card-lg overflow-hidden animate-in fade-in duration-500">
          <div className="flex justify-end mb-4 no-print">
            <button
              onClick={generatePDF}
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 shadow-card"
            >
          ...
          <div className="mb-6 space-y-1">
            ...
            <p className="mt-4 font-semibold">
              Solde actuel : {Number(data.soldeActuel).toLocaleString("de-DE")}{" "}
              Ariary
            </p>
          </div>
