...
import { Settings, X, Save, Edit, Trash2, Plus, BarChart3, Coins } from 'lucide-react';
...
const FraisEnvoiList = () => {
  ...
  const fetchAllFrais = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/v1/FraisEnvoi/all`, { headers });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`http://localhost:8080/api/v1/FraisEnvoi/FraisEnvoi/${formData.idEnv}/update`, formData, { headers });
        toast.success("Tarif mis à jour !");
      } else {
        await axios.post(`http://localhost:8080/api/v1/FraisEnvoi/add`, formData, { headers });
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:8080/api/v1/FraisEnvoi/FraisEnvoi/${fraisToDelete}/delete`, { headers });
  ...
  return (
    <div className="ml-64 p-8 bg-[#0f172a] min-h-screen flex-1 text-slate-200">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Configuration des Frais</h1>
          <p className="text-slate-500 text-sm italic">Définition des tarifs par tranche de montant</p>
        </div>

        <button 
          onClick={openAddModal}
          className="bg-amber-600 hover:bg-amber-700 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-lg shadow-amber-900/20 transition-all transform active:scale-95"
        >
      ...
      <div className="bg-[#1e293b] rounded-2xl border border-slate-700 shadow-2xl overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-700">
            {frais.map((f) => (
              <tr key={f.idEnv} className="hover:bg-slate-800/40 transition-colors group">
                ...
                <td className="px-6 py-4 font-bold text-amber-500">{f.frais_env.toLocaleString()} Ar</td>
                <td className="px-6 py-4">
                  <div className="flex justify-center gap-3">
                    ...
                    <button onClick={() => triggerDelete(f.idEnv)} className="p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition-all"><Trash2 size={16}/></button>
                  </div>
                </td>
              </tr>
            ))}
          ...
          <div className="bg-[#1e293b] w-full max-w-md rounded-3xl border border-slate-700 shadow-2xl animate-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-700 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                {isEditMode ? <Settings size={20} className="text-amber-500"/> : <Plus size={20} className="text-amber-500"/>}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="number" value={formData.montant1} className="w-full bg-[#0f172a] border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-amber-500" onChange={(e) => setFormData({...formData, montant1: e.target.value})} />
                </div>
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 ml-1">Max (Ar)</label>
                  <input required type="number" value={formData.montant2} className="w-full bg-[#0f172a] border border-slate-700 rounded-xl p-3 text-white outline-none focus:border-amber-500" onChange={(e) => setFormData({...formData, montant2: e.target.value})} />
                </div>
              </div>

              <div>
                <label className="text-[10px] uppercase font-bold text-slate-500 ml-1 text-amber-400">Valeur des frais (Ar)</label>
                <div className="relative">
                  <Coins size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-amber-500" />
                  <input required type="number" value={formData.frais_env} className="w-full bg-[#0f172a] border border-slate-700 rounded-xl p-3 pl-10 text-amber-400 font-bold text-xl outline-none focus:border-amber-500" onChange={(e) => setFormData({...formData, frais_env: e.target.value})} />
                </div>
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  {loading ? "Chargement..." : <><Save size={18} /> {isEditMode ? 'Mettre à jour' : 'Enregistrer le tarif'}</>}
                </button>
              </div>
            </form>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
          <div className="bg-[#1e293b] w-full max-w-sm rounded-3xl border border-slate-700 shadow-2xl p-8 text-center animate-in zoom-in duration-200">
            ...
            <p className="text-slate-400 text-sm mb-8">Cela pourrait affecter les calculs des prochains transferts.</p>
